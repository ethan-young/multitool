...
The `model_parameters` column gives you the result of calling `parameters::parameters()` on each model in your grid, which is a `data.frame` of model coefficients and their associated standard errors, confidence intervals, test statistic, and a p-values.
...
`.how` will create a column named like so `<column being condsensed>_<summary function name provided>"`. For this case, we have `r2`_mean` and `r2_median`.
